...
<div class="max-w-3xl mx-auto py-12" dir="rtl">
        <h1 class="text-2xl font-bold text-[#0F4C81] dark:text-[#93C5FD] m-0 mb-6">تعديل خدمة</h1>
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-[#374151] dark:bg-[#1F2937]">
            <form method="POST" action="{{ route('services.update', $service) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" max="100" name="doctor_percentage" id="doctor_percentage" value="{{ old('doctor_percentage', $service->doctor_percentage) }}"
                        class="block w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm dark:border-[#374151] dark:bg-[#111827] dark:text-[#F3F4F6]">
                ...
                <div class="flex justify-end gap-3 pt-4">
                    ...
                    <button type="submit" class="rounded-lg bg-[#0F4C81] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#0c3d66]">تحديث</button>
                </div>
            </form>
        </div>
    </div>
