...
<div class="mx-auto w-full max-w-[1100px]" dir="{{ $uiRtl ? 'rtl' : 'ltr' }}">
    <div class="mb-6">
        ...
        <h1 class="m-0 text-[28px] font-bold text-[#0F4C81] dark:text-[#93C5FD]">{{ __('invoices.page_title_create') }}</h1>
        <p class="m-0 mt-2 text-sm text-gray-600 dark:text-[#9CA3AF]">{{ __('invoices.create_subtitle') }}</p>
    </div>

    <x-invoice.nav active="create" />
        ...
        <div class="p-4 sm:p-6">
            <form method="POST" action="{{ route('invoices.store') }}" id="invoiceForm">
                            ...
                            <div>
                                ...
                                <select id="patient_id" name="patient_id" class="{{ $inputClass }}" required>
                                    ...
                                    @foreach($patients as $patient)
                                        <option value="{{ $patient->id }}" {{ (string) old('patient_id', request('patient_id')) === (string) $patient->id ? 'selected' : '' }}>{{ $patient->full_name }}</option>
                                    @endforeach
                                </select>
                                @error('patient_id')
                            ...
                            <div>
                                ...
                                <select id="visit_id" name="visit_id" class="{{ $inputClass }}">
                                    ...
                                    @foreach($visits as $visit)
                                        <option value="{{ $visit->id }}" {{ (string) old('visit_id') === (string) $visit->id ? 'selected' : '' }}>{{ $visit->chief_complaint ?: __('invoices.visit_fallback_label') }} — {{ optional($visit->patient)->full_name ?? __('common.em_dash') }}</option>
                                    @endforeach
                                </select>
                                @error('visit_id')
                                    <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            ...
                            <div id="itemsContainer">
                                <div class="item-row grid grid-cols-1 md:grid-cols-12 gap-4 mb-4 items-end">
                                    <div class="md:col-span-3">
                                        ...
                                        <select name="items[0][service_id]" class="service-select {{ $inputClass }}">
                                            <option value="">{{ __('invoices.service_manual') }}</option>
                                            @foreach($services as $svc)
                                                <option value="{{ $svc->id }}" data-price="{{ $svc->price }}">{{ $svc->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="md:col-span-3">
                                        <label class="{{ $itemLabelClass }}">{{ __('invoices.label_item_name') }}</label>
                                        <input type="text" name="items[0][item_name]" placeholder="{{ __('invoices.placeholder_item_name') }}" class="{{ $inputClass }}" required>
                                    </div>
                                    <div class="md:col-span-2">
                                        <label class="{{ $itemLabelClass }}">{{ __('invoices.label_price') }}</label>
                                        <input type="number" step="0.01" name="items[0][price]" placeholder="{{ __('invoices.placeholder_price') }}" class="price-input {{ $inputClass }}" required>
                                    </div>
                                    <div class="md:col-span-2">
                                        <label class="{{ $itemLabelClass }}">{{ __('invoices.label_quantity') }}</label>
                                        <input type="number" min="1" name="items[0][quantity]" placeholder="{{ __('invoices.placeholder_quantity') }}" class="quantity-input {{ $inputClass }}" required>
                                    </div>
                                    <div class="md:col-span-2 flex items-center gap-2 pb-1">
                                        ...
                                        <button type="button" class="{{ $removeItemBtnClass }}" style="display: none;">{{ __('invoices.btn_remove_line') }}</button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="mb-6 rounded-lg border border-slate-200/90 bg-slate-50/80 px-4 py-3 dark:border-[#374151] dark:bg-[#111827]/60">
                            <p class="m-0 text-base font-semibold text-slate-800 dark:text-slate-100">{{ __('invoices.total_preview') }} <span id="totalAmount" class="tabular-nums text-[#0F4C81] dark:text-[#93C5FD]">0.00</span></p>
                        </div>

                        <div class="flex flex-wrap justify-end gap-3 border-t border-gray-100 pt-6 dark:border-[#374151]">
                            <a href="{{ route('invoices.index') }}" data-spa class="inline-flex items-center justify-center rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50 dark:border-[#4B5563] dark:bg-[#1F2937] dark:text-[#E5E7EB]">{{ __('common.cancel') }}</a>
                            <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-[#0F4C81] px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-[#0c3d66] dark:bg-[#3B82F6]">{{ __('invoices.save_invoice') }}</button>
                        </div>
                    </form>
        </div>
    </div>
