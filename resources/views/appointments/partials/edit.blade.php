...
<div class="mx-auto w-full max-w-[900px]" dir="{{ $apptDir }}">
    <div class="mb-6">
        ...
        <h1 class="m-0 text-[28px] font-bold text-[#0F4C81] dark:text-[#93C5FD]">{{ __('appointments.heading_edit') }}</h1>
        <p class="m-0 mt-2 text-sm text-gray-600 dark:text-[#9CA3AF]">
            {{ optional($appointment->patient)->full_name ?? __('common.em_dash') }}
            · @safeDate($appointment->appointment_date)
